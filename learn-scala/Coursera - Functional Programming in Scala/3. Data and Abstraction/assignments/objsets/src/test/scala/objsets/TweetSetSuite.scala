...
class TweetSetSuite extends munit.FunSuite:
  ...
  test("filter: twenty on set5") {
    new TestSets:
      println(asSet(set5.filter(tw => tw.retweets == 20))) // Check which tweets are in the filtered result
      assertEquals(size(set5.filter(tw => tw.retweets == 20)), 2)
  }

  test("union: set4c and set4d") {
    new TestSets:
      println(asSet(set4c.union(set4d))) // Check which tweets are in the union result
